...
public class GuiDaoZaiInventory extends InventoryEffectRenderer
{
    ...
    protected void actionPerformed(GuiButton button)
    {
        if (button.id == 0 || button.id == 1)
        {
            ...
            if (button.id == 0) Minecraft.getMinecraft().displayGuiScreen(new GuiNewInventory(player));
            if (button.id == 1) Minecraft.getMinecraft().displayGuiScreen(new GuiInwardObserve(player));
        }
    }
