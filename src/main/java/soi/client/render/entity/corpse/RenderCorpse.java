...
package soi.client.render.entity.corpse;
...
public class RenderCorpse extends Render<EntityCorpse>
{
    ...
    public RenderCorpse(RenderManager render)
    {
        super(render);
    ...
    public void doRender(EntityCorpse entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        if (entity.getCorpseAge() >= 20 * 25 * 60)
        {
    ...
    {
        return PlayerSkin.getSkin(entity.getCorpseUUID(), entity.getCorpseName());
    }

    public boolean isSlim(UUID uuid)
    {
        NetworkPlayerInfo info = mc.getConnection().getPlayerInfo(uuid);
        return info == null ? (uuid.hashCode() & 1) == 1 : info.getSkinType().equals("slim");
    }
}