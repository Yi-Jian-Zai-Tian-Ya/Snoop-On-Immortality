...
public class EntityCorpse extends EntityCorpseInventoryBase
{
    ...
    public void onUpdate()
    {
        ...
        if (!collidedVertically && posY > 0D) motionY = Math.max(-2D, motionY - 0.0625D);
        else motionY = 0D;

        if (posY < 0D) setPositionAndUpdate(posX, 0F, posZ);

        move(MoverType.SELF, motionX, motionY, motionZ);

        if (world.isRemote) return;

        if ((isEmpty() && getCorpseAge() > 600) || (getCorpseAge() > 36000)) setDead();
    ...
    public boolean processInitialInteract(EntityPlayer player, EnumHand hand)
    {
        if (!world.isRemote && player instanceof EntityPlayerMP)
        {
            ...
            player.openGui(SOI.MODID, CommonProxy.GUI_CORPSE, player.world, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
    ...
    public ITextComponent getDisplayName()
    {
        ...
        {
            return new TextComponentTranslation("entity.soi.corpse_of", getCorpseName());
        }
    }
